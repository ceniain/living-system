...
{
    int fd;
    time_t lastBeatTime;  // 最后一次收到心跳的时间戳
}ClientContext;
...
public:
    ...
    //epoll事件循环
    void EventLoop();
    void ProcessOnce();       // 新增：单次非阻塞轮询
    static Block_Epoll_Net* GetInstance();
    //发送数据
...
private:
    ...
    //房间-在线客户端fd列表
public:
    const int HEART_BEAT_TIMEOUT = 10;// 心跳超时阈值：10秒（可自行修改）
    // fd 映射到心跳上下文
    std::map<unsigned long long, std::vector<sock_fd>> m_RoomClientMap;
public:
    //心跳检测函数声明
        void CheckHeartBeatTimeout();
        void UpdateHeartBeatTime(int fd);

    //客户端加入房间
    void AddClientToRoom(unsigned long long roomNo,sock_fd fd);
    //清空房间所有客户端
    void ClearRoomClient(unsigned long long roomNo);
    //房间内群发数据
    ...
    pthread_mutex_t m_beatMutex;
    // ===================== 单例私有构造/禁用拷贝 =====================
        Block_Epoll_Net()
        {
            pthread_mutex_init(&m_beatMutex, nullptr);
        }
        ~Block_Epoll_Net()
        {
            pthread_mutex_destroy(&m_beatMutex);
        }
        Block_Epoll_Net(const Block_Epoll_Net&) = delete;
        Block_Epoll_Net& operator=(const Block_Epoll_Net&) = delete;

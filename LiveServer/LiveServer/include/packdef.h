...
#define _DEF_PACK_ROOM_COUNT_BROADCAST    (_DEF_PACK_BASE + 18)//房间在线人数广播
...
#define _DEF_PACK_LEAVE_ROOM_RQ        (_DEF_PACK_BASE + 26)//退房请求路由
#define _DEF_PACK_LEAVE_ROOM_RS        (_DEF_PACK_BASE + 27)//退房路由
...
//=================弹幕协议=================
#define danmu_success 1
typedef struct STRU_DANMU_RQ
{
    STRU_DANMU_RQ():type(_DEF_PACK_SEND_DANMU_RQ)
    {
        memset(msg,0,sizeof(msg));
        memset(username, 0, sizeof(username));  // 初始化用户名
        userid = 0;
...
typedef struct STRU_DANMU_RS
{
    STRU_DANMU_RS():type(_DEF_PACK_DANMU_BROADCAST),result(danmu_success)
    {
        memset(msg,0,sizeof(msg));
        memset(username, 0, sizeof(username));  // 初始化用户名
        userid = 0;
...
{
    STRU_STOP_LIVE_RS() : type(_DEF_PACK_STOP_LIVE_RS), result(1)
    {}
    PackType type;
    int result;
...
typedef struct STRU_ROOM_COUNT_BROADCAST
{
    STRU_ROOM_COUNT_BROADCAST()
        : type(_DEF_PACK_ROOM_COUNT_BROADCAST),
          room_no(0),
          count(0)
    {}
...
// 用户所在房间 <fd, 房间号>
extern std::map<int, unsigned long long> g_userRoomMap;

...
#define FILE_NAME "D:/MediaPlayer/video/riku.mp4"
...
int main(int argc, char *argv[])
{
    ...
    while(av_read_frame(pFormatCtx, &packet) >= 0) //读一个 packet，数据放在 packet.data
    {
        ...
        {
            packet_queue_put(&audio_queue, &packet);//只把音频流放入队列
        }
        else
        {
...
int audio_decode_frame(AVCodecContext *pcodec_ctx, uint8_t *audio_buf, int buf_size)
{
    ...
    for(;;)
    {
        ...
        while(audio_pkt_size > 0)
        {
            ...
            int ret =avcodec_decode_audio4( aCodecCtx, audioFrame, &got_picture, &pkt);//解码音频
            if( ret < 0 ) {

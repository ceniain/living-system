...
SaveVideoFileThread::SaveVideoFileThread()
{
    ...
    connect(m_picinpicRead,SIGNAL(SIG_sendPicInPic(QImage)),
            this,SIGNAL(SIG_sendPicInPic(QImage)));
    connect(m_picinpicRead,SIGNAL(SIG_sendVideoFrame(QImage)),
            this,SIGNAL(SIG_sendVideoFrame(QImage)));

    connect(m_picinpicRead,SIGNAL(SIG_sendVideoFrameData(uint8_t*,int)),
            this,SLOT(slot_writeVideoFrameData(uint8_t*,int)));
...
/* Add an output stream. */
void SaveVideoFileThread::add_video_stream(OutputStream *ost,
                                     ...
                                     AVStream *st, AVFrame *frame)
{
    if (c->codec_type == AVMEDIA_TYPE_AUDIO) {
        qDebug() << "========== 【测试】write_frame 音频被调用了！==========";
    }
    int ret;

    if (!frame) return 0;
    //    if (c->codec_type == AVMEDIA_TYPE_AUDIO) {
    //        qDebug() << "【音频】正在编码音频帧！PTS:" << frame->pts;
    //    }
    // send the frame to the encoder
    ret = avcodec_send_frame(c, frame);
    if (ret < 0) {
        fprintf(stderr, "Error sending a frame to the encoder\n");
        qDebug() << "=== 🚨 错误：H.264 编码器拒绝接收这一帧！ ===";
        return 0;
    }
    ...
    while (ret >= 0) {
        ...
        //qDebug() << "➔ 编码器成功出货！生成 H.264 Packet -> 原始 PTS:" << pkt.pts << "DTS:" << pkt.dts;
        /* rescale output packet timestamp values from codec to stream timebase */
        av_packet_rescale_ts(&pkt, c->time_base, st->time_base);
        pkt.stream_index = st->index;

        //qDebug() << "   └─► 经过转换后准备写入文件的 Packet -> 转换后 PTS:" << pkt.pts << "DTS:" << pkt.dts;
        /* Write the compressed frame to the media file. */
        ret = av_interleaved_write_frame(fmt_ctx, &pkt);
        av_packet_unref(&pkt);
        if (ret < 0) {
            qDebug() << "   ❌ 致命错误：av_interleaved_write_frame 写入文件失败，返回值 ret =" << ret;
...
void SaveVideoFileThread::slot_writeVideoFrameData(uint8_t *picture_buf, int buffer_size)
{
    ...
    //写入视频帧  picture_buf -> ost->frameBuffer
...
void SaveVideoFileThread::slot_setInfo(STRU_AV_FORMAT &avFormat)
{
    ...
    avformat_alloc_output_context2(&oc, NULL, "flv", filename);//保存flv格式
    //为了未来推流rtmp(flv)
    if (!oc) {

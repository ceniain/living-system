...
#define _DEF_PACK_STOP_LIVE_RS        (_DEF_PACK_BASE + 17)  // 关播回包
...
{
    STRU_CREATE_ROOM_RS():type(_DEF_PACK_CREATE_ROOM_RS), result(0), room_no(0) {}
    PackType type;
    int result;
    unsigned long long room_no;   // 🔥 8位对外房间号（观众用）
}STRU_CREATE_ROOM_RS;
...
typedef struct STRU_JOIN_ROOM_RQ
{
    ...
    PackType type;
    int userid;
    unsigned long long room_no;  // 🔥 观众输入 8 位房间号
}STRU_JOIN_ROOM_RQ;
...
{
    STRU_STOP_LIVE_RS() : type(_DEF_PACK_STOP_LIVE_RS), result(stop_success) {}
    PackType type;
    int result;
}STRU_STOP_LIVE_RS;
// 禁言操作类型

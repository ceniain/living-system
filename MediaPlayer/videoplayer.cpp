...
#include <signal.h> // 可选，确保time相关函数可用
    ...
    while(1)
    {
        ...
        if (packet_queue_get(is->videoq, packet, 0) <= 0)
        {
            if( (is->seek_flag_video !=1)
                    && is->readFinished && is->audioq->nb_packets == 0)//播放到结束
            {//读线程完毕
                break;
            }else
            {
        ...
        while(1)
        {
            if(is->quit) break;//关闭播放退出
            //防止没声音无法退出同步
            if(is->audioq->size==0) break;//音频队列为空退出
            audio_pts = is->audio_clock;//音频时钟
            video_pts = is->video_clock;//视频时钟
            //如果视频帧的时间戳已经小于等于音频的时间戳，说明视频已经追上了音频，
            //这一帧可以正常显示了，循环结束，继续执行视频显示逻辑
...
void VideoPlayer::run()
{
    ...
    avdevice_register_all();   // 注册设备（网络流需要）
...
//    }
    ...
    if( res < 0 )
    {
        ...
        m_videoState.readThreadFinished = true; // 标记线程结束
        m_playerState = PlayerState::Stop; // 置为停止状态
        stop(true); // 执行stop逻辑
        return;
    }else
    {
        input_runner.connected = true; // 连接成功，标记为true
    }
    //3.1 获取视频文件信息
    ...
    if (audioStream != -1) {
        ...
        //9.设置参数，供解码时候用, swr_alloc_set_opts 的 in 部分参数
        switch (pFormatCtx->streams[audioStream]->codec->sample_fmt)
        {
        case AV_SAMPLE_FMT_U8:
...
不是包的数量 */
        ...
        //10 然后 10-13 的包全扔掉
        {
            ...
            m_videoState.seek_time = m_videoState.seek_pos ; //精确到微妙 seek_time 是用来做频音频的时钟调整 --关键帧
            m_videoState.seek_flag_audio = 1; //在视频音频循环中 , 判断, AVPacket 是 FLUSH_DATA清空解码器缓存
            m_videoState.seek_flag_video = 1;
        }
        if (av_read_frame(pFormatCtx, packet) < 0)
        {//读不到视频帧时不是立即退出，而是等一段时间
            DelayCount++;//记录读取失败的次数
            if( DelayCount>= 300)
            {//连续读取失败的次数超过 300 次，就标记 m_videoState.readFinished = true;，表示 “文件读取结束”
